...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		int victimSwap = swap_pageout(frame, coremap[frame].pte->swap_off); 
		coremap[frame].pte->swap_off = victimSwap;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	pgdir_entry_t currentEntry = pgdir[idx]; 
	uintptr_t almostSecondLevelPageTable = currentEntry.pde; 
	if(!(almostSecondLevelPageTable & PG_VALID)){
		pgdir[idx] = init_second_level();
		almostSecondLevelPageTable = pgdir[idx].pde; 
	}

	pgtbl_entry_t* secLevPgTblNum = (pgtbl_entry_t*)(almostSecondLevelPageTable & ~PG_VALID);
	
	unsigned idSecondLevelIndex = PGTBL_INDEX(vaddr);
	pgtbl_entry_t* PTEwewant = secLevPgTblNum + idSecondLevelIndex; 
	...
	if(p->frame & PG_VALID){
		...
		// mark dirty if the access type is M or S 
		if(type == 'M' || type == 'S'){
		...
		// frame is not valid. it may be on swap file or not on swap file 
		if(p->frame & PG_ONSWAP){
			// frame is nto valid. it is ON the swap file 
			int newlyAllocatedFrame = allocate_frame(p); 
			
			int swapResult = swap_pagein(newlyAllocatedFrame, p->swap_off); 
			newlyAllocatedFrame = newlyAllocatedFrame << PAGE_SHIFT; 
			if(swapResult != 0){
				exit(1); 
			}
			p->frame = newlyAllocatedFrame;
			// unset the pg_onswap bit 
			...
			// frame is not valid. it is not on the swap file. 
			int newlyAllocatedFrame = allocate_frame(p); 
			
			init_frame(newlyAllocatedFrame, vaddr); 
			newlyAllocatedFrame = newlyAllocatedFrame << PAGE_SHIFT; 
			p->frame = newlyAllocatedFrame;
			p->frame = p->frame | PG_VALID; 
			p->frame = p->frame | PG_REF; 
